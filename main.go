...
import (
	"context"
	dpfm_api_caller "data-platform-api-business-partner-creates-general-rmq-kube/DPFM_API_Caller"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	errs := caller.AsyncBusinessPartnerCreates(accepter, &input, &output, l)
	if len(errs) != 0 {
